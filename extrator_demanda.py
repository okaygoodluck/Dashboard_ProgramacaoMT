        ...
        try:
            ...
            for malha in lista_malhas:
                ...
                for regiao_obj in lista_regioes_objs:
                    ...
                    # Salva a região descoberta (para garantir que ela sempre exista no Dashboard, mesmo sem dados)
                    todas_regioes_sistema.append({'Ref_Malha': malha, 'Ref_Regiao': regiao})
                    ...
                    # NOVA LÓGICA: Double-Check e Espera de Mensagem (Camadas 1 e 2)
                    max_tentativas_vazio = 2
                    tentativa_vazio = 0
                    tem_dados = False
                    confirmado_vazio = False

                    while tentativa_vazio < max_tentativas_vazio:
                        ...
                        if tentativa_vazio == 0:
                            print("    -> Pesquisando...")
                        else:
                            print(f"    -> Tabela aparentemente vazia. Realizando double-check (tentativa {tentativa_vazio + 1})...")
                        ...
                        if tem_dados:
                            break # Encontrou dados, sai do loop de tentativas
                            
                        # Verifica mensagem explícita de "Nenhum registro encontrado"
                        try:
                            confirmado_vazio = ctx_tabela.evaluate("""() => {
                                const msg = document.querySelector('.rich-messages, .messages, .rf-msgs');
                                if (msg && msg.innerText.toLowerCase().includes('nenhum')) return true;
                                const spans = Array.from(document.querySelectorAll('span, td'));
                                const found = spans.find(el => {
                                    const txt = el.innerText.toLowerCase();
                                    return txt.includes('nenhum registro') || txt.includes('nenhuma ocorrencia') || txt.includes('não foram encontrados');
                                });
                        ...
                        if confirmado_vazio:
                            print("    -> Confirmação explícita do sistema: Nenhum registro encontrado.")
                            break # Vazio confirmado, sai do loop

                        # Se não tem dados e não tem mensagem, pode ser falha. Vamos tentar de novo.
                        tentativa_vazio += 1
                        if tentativa_vazio < max_tentativas_vazio:
                            time.sleep(2.0) # Pausa antes de tentar de novo

                    if not tem_dados:
                        if confirmado_vazio:
                            print("    -> Sem registros nesta região (confirmado).")
                            regioes_confirmadas_vazias.append(regiao)
                        else:
                            print("    -> [ALERTA] Região sem registros, mas sem confirmação explícita do sistema. Possível falha de carregamento.")
                        continue
            ...
            if dados_consolidados:
                ...
                try:
                    ...
                    if arquivos_mesao:
                        ...
                        if col_solic_mesao and col_regiao_mesao and col_solic_df:
                            ...
                            def calcular_prioridade(row):
                                ...
                                # Se a solicitação está no Mesão E a região extraída bate com a do Mesão = Prioridade Máxima (1)
                                if solic in mapa_regioes:
                                    ref_reg = str(row.get('Ref_Regiao', '')).strip().upper()
                                    if ref_reg.startswith(mapa_regioes[solic]):
                                        return 1
                                # Caso contrário, Prioridade Baixa (0)
                                return 0
                            
                            df['_prioridade'] = df.apply(calcular_prioridade, axis=1)
                            
                            # 2. Ordenamos para que as Prioridades Máximas fiquem no topo
                            df = df.sort_values(by='_prioridade', ascending=False)
                            
                            # 3. Dropamos as duplicatas pelo número da solicitação, mantendo a primeira (que será a de maior prioridade)
                            df = df.drop_duplicates(subset=[col_solic_df], keep='first')
                            
                            # 4. Limpamos as colunas temporárias
                            df = df.drop(columns=['_solic_key', '_prioridade'])
                    ...
                    # Limpeza de fallback caso ocorra erro no meio do processo
                    for col in ['_solic_key', '_prioridade']:
                ...
                try:
                    ...
                    if col_urg and col_solic:
                        ...
                        if ids_urgentes:
                            ...
                            # Chama o PowerShell silenciosamente com trava de 60 segundos
                            ...
                            try:
                                ...
                                print("    -> [ALERTA] Timeout: O Outlook demorou mais de 60s para responder. Ignorando e-mails para evitar travamento.")
                                df['Tem_Email'] = False
                        else:
                ...
                try:
                    ...
                    db_manager.salvar_dados(df, regioes_confirmadas_vazias)
                    
                    if todas_regioes_sistema:
                        print("[DB] Salvando mapeamento completo de regiões do sistema...")
                        db_manager.salvar_regioes_sistema(pd.DataFrame(todas_regioes_sistema))
