...
def _get_path(filename, network_path):
...
def get_data_db_path():
    ...
    p = _get_path(DB_DATA_NAME, REDE_DATA_PATH)
    return p if p else LOCAL_DB_NAME # Fallback para o local se nada for encontrado

def publicar_db_rede():
    """Copia o arquivo local 'demanda.db' para a rede como 'vanguard_data.db'"""
    import shutil
    try:
...
def get_connection_config():
    """Conexão para sistema (Usuários/Config). Prioriza 'vanguard_app.db' na rede."""
